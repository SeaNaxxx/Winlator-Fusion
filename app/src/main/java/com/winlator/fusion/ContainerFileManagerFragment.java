...
import com.winlator.fusion.container.FileInfo;
import com.winlator.fusion.contentdialog.ContentDialog;
import com.winlator.fusion.contentdialog.FileInfoDialog;
...
import com.winlator.fusion.win32.MSIcon;
import com.winlator.fusion.win32.MSLink;
import com.winlator.fusion.win32.PEParser;
...
public class ContainerFileManagerFragment extends BaseFileManagerFragment<FileInfo> {
    ...
    public boolean onOptionsItemSelected(MenuItem menuItem) {
        ...
        if (id == R.id.menu_item_home) {
            ...
            refreshContent();
            return true;
        } else if (id == R.id.menu_item_view_style) {
            ...
            return true;
        } else if (id == R.id.menu_item_new_folder) {
            createFolder();
            return true;
        } else {
    ...
    private class FileInfoAdapter extends RecyclerView.Adapter<FileInfoAdapter.ViewHolder> {
        ...
        private void showListItemMenu(View anchorView, final FileInfo file) {
            ...
            listItemMenu.setOnMenuItemClickListener((menuItem) -> {
                int id = menuItem.getItemId();
                if (id == R.id.menu_item_copy || id == R.id.menu_item_cut) {
                    instantiateClipboard(file, id == R.id.menu_item_cut);
                ...
                } else if (id == R.id.menu_item_rename) {
                    clearClipboard();
                    ContentDialog.prompt(context, R.string.rename, file.name, (newName) -> {
                        file.renameTo(newName);
                        refreshContent();
                    });
                } else if (id == R.id.menu_item_add_favorite) {
                    addFavorite(file);
