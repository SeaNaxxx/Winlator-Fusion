...
import com.winlator.fusion.fexcore.FEXCoreEditPresetDialog;
...
import com.winlator.fusion.contentdialog.GamepadPlayerConfigDialog;
import com.winlator.fusion.contentdialog.SoundFontTestDialog;
...
import com.winlator.fusion.widget.LogView;
...
public class SettingsFragment extends Fragment {
    public static final String DEFAULT_WINLATOR_PATH = android.os.Environment.getExternalStorageDirectory().getPath() + "/Winlator";
    ...
    public void onActivityResult(int requestCode, int resultCode, @Nullable Intent data) {
        if (requestCode == MainActivity.OPEN_FILE_REQUEST_CODE && resultCode == Activity.RESULT_OK) {
            try {
                ...
                else if (selectPresetFileCallback != null && data != null) selectPresetFileCallback.call(data.getData());
            }
            catch (Exception e) {
    ...
    public View onCreateView(@NonNull LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        ...
        view.findViewById(R.id.BTReinstallSystemFiles).setOnClickListener((v) -> {
            ContentDialog.confirm(context, R.string.do_you_want_to_reinstall_system_files, () -> FusionFSInstaller.install((MainActivity)getActivity()));
        });

        final AppCompatActivity activity = (AppCompatActivity)getActivity();

        view.findViewById(R.id.BTBackupData).setOnClickListener((v) -> {
            ContentDialog.confirm(context, R.string.do_you_want_to_create_backup, () -> {
                preloaderDialog.show(R.string.creating_backup);
                Executors.newSingleThreadExecutor().execute(() -> {
                    try {
                        File dataDir = context.getFilesDir();
                        File backupFile = new File(Environment.getExternalStorageDirectory(), "winlator_fusion_backup.tar");
                        TarCompressorUtils.archive(dataDir, backupFile, (dir, name) -> {
                            return !name.equals(".sysvshm");
                        });
                        activity.runOnUiThread(() -> {
                            preloaderDialog.close();
                            AppUtils.showToast(context, R.string.backup_completed);
                        });
                    } catch (Exception e) {
                        activity.runOnUiThread(() -> {
                            preloaderDialog.close();
                            AppUtils.showToast(context, R.string.backup_failed);
                        });
                    }
                });
            });
        ...
        view.findViewById(R.id.BTRestoreData).setOnClickListener((v) -> {
            ContentDialog.confirm(context, R.string.do_you_want_to_restore_backup, () -> {
                preloaderDialog.show(R.string.restoring_backup);
                Executors.newSingleThreadExecutor().execute(() -> {
                    try {
                        File dataDir = context.getFilesDir();
                        File backupFile = new File(Environment.getExternalStorageDirectory(), "winlator_fusion_backup.tar");
                        if (!backupFile.isFile()) {
                            activity.runOnUiThread(() -> {
                                preloaderDialog.close();
                                AppUtils.showToast(context, R.string.restore_failed);
                            });
                            return;
                        }
                        ...
                        if (children != null) {
                            for (String child : children) {
                                FileUtils.delete(new File(dataDir, child));
                            }
                        }
                        TarCompressorUtils.extract(backupFile, dataDir);
                        activity.runOnUiThread(() -> {
                            preloaderDialog.close();
                            AppUtils.showToast(context, R.string.restore_completed);
    ...
    private void loadBox64PresetSpinner(View view, final Spinner sBox64Preset) {
        ...
        view.findViewById(R.id.BTExportBox64Preset).setOnClickListener((v) -> {
            ...
            Box64PresetManager.exportPreset(context, presetId);
        });
        view.findViewById(R.id.BTImportBox64Preset).setOnClickListener((v) -> {
            selectPresetFileCallback = (uri) -> {
                try {
                    InputStream stream = context.getContentResolver().openInputStream(uri);
                    if (stream != null) {
                        Box64PresetManager.importPreset(context, stream);
                        stream.close();
                        updateSpinner.run();
                        sBox64Preset.setSelection(sBox64Preset.getCount()-1);
                    }
    ...
    private void loadFEXCorePresetSpinner(View view, final Spinner sFEXCorePreset) {
        final Context context = getContext();

        Runnable updateSpinner = () -> {
            FEXCorePresetManager.loadSpinner(sFEXCorePreset, preferences.getString("fexcore_preset", FEXCorePreset.DEFAULT));
        };
        ...
        view.findViewById(R.id.BTImportFEXCorePreset).setOnClickListener((v) -> {
            selectPresetFileCallback = (uri) -> {
                try {
                    InputStream stream = context.getContentResolver().openInputStream(uri);
                    if (stream != null) {
                        FEXCorePresetManager.importPreset(context, stream);
                        stream.close();
                        updateSpinner.run();
                        sFEXCorePreset.setSelection(sFEXCorePreset.getCount()-1);
                    }
    ...
    private void removeInstalledWine(WineInfo wineInfo, Runnable onSuccess) {
        ...
        File installedWineDir = FusionFS.find(activity).getInstalledWineDir();
    ...
    public static void resetFEXCoreVersion(AppCompatActivity activity) {
        SharedPreferences preferences = PreferenceManager.getDefaultSharedPreferences(activity);
        SharedPreferences.Editor editor = preferences.edit();
        editor.putString("fexcore_version", DefaultVersion.FEXCORE);
