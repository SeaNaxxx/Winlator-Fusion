...
public class Container {
    ...
    public static final String DEFAULT_ENV_VARS_BIONIC = "ZINK_DESCRIPTORS=lazy ZINK_DEBUG=compact WRAPPER_MAX_IMAGE_COUNT=0 RENDERER_SWAPCHAIN=0 VKD3D_SHADER_MODEL=6_6 TU_DEBUG=noconform MESA_SHADER_CACHE_DISABLE=false MESA_SHADER_CACHE_MAX_SIZE=512MB mesa_glthread=true WINEESYNC=1 DXVK_HUD=/0";
    ...
    private String graphicsDriver = GraphicsDrivers.DEFAULT_VULKAN_DRIVER_BIONIC+","+GraphicsDrivers.DEFAULT_OPENGL_DRIVER_BIONIC;
    ...
    private int primaryController = 0;
    ...
    public String getExtraData() {
        return extraData != null ? extraData.toString() : "";
    }
        ...
        for (Iterator<String> it = data.keys(); it.hasNext(); ) {
            ...
            switch (key) {
                ...
                case "primaryController":
                    primaryController = data.getInt(key);
                    break;
    ...
    private void validateVariantDefaults() {
        if (isBionic()) {
            ...
            if (box64Version.equals(DefaultVersion.BOX64)) {
                box64Version = DefaultVersion.BOX64_BIONIC;
            }
            ...
            boolean isArm64EC = wineVersion != null && wineVersion.contains("arm64ec");
            if (!isArm64EC) {
                if (emulator.equals("FEXCore")) {
                    emulator = "Box64";
                }
            }
        } else {
    ...
    public static void checkObsoleteOrMissingProperties(JSONObject data) {
        try {
            if (data.has("envVars")) {
                ...
                if (shouldPatch) {
                    EnvVars defaultEnvVars = new EnvVars(isBionic ? DEFAULT_ENV_VARS_BIONIC : DEFAULT_ENV_VARS);
