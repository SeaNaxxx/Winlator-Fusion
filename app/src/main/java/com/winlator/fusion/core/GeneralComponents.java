...
public abstract class GeneralComponents {
    ...
    private static final String INSTALLABLE_COMPONENTS_URL = "https://raw.githubusercontent.com/SeaNaxxx/winlator/dev/installable_components/%s";

    public enum Type {
        BOX64, TURNIP, DXVK, VKD3D, WINED3D, SOUNDFONT, ADRENOTOOLS_DRIVER, FEXCORE;
        ...
        public File getDestination(Context context) {
            return getDestination(context, RootFS.find(context).getRootDir());
        }

        public File getDestination(Context context, File rootDir) {
            switch (this) {
                case DXVK:
                case VKD3D:
                case WINED3D:
                ...
                case SOUNDFONT:
                    File destination = new File(context.getCacheDir(), "soundfonts");
        ...
        private boolean isVersioned() {
            return this == BOX64 || this == TURNIP || this == DXVK || this == VKD3D || this == WINED3D || this == FEXCORE;
        }
    }
    ...
    public static ArrayList<String> getBuiltinComponentNames(Type type) {
        ...
        switch (type) {
            ...
            case ADRENOTOOLS_DRIVER:
                ...
                break;
            case FEXCORE:
                items = new String[]{DefaultVersion.FEXCORE};
                break;
        }
    ...
    public static void extractFile(Type type, Context context, String identifier, String defaultVersion, TarCompressorUtils.OnExtractFileListener onExtractFileListener) {
        File destination = type.getDestination(context);
        doExtractFile(type, context, identifier, defaultVersion, destination, onExtractFileListener);
    }

    /**
     * Extract a component file to a specific destination directory.
     * Use this for Bionic containers where the destination is imagefs, not rootfs.
     */
    public static void extractFile(Type type, Context context, String identifier, String defaultVersion, File destinationDir, TarCompressorUtils.OnExtractFileListener onExtractFileListener) {
        doExtractFile(type, context, identifier, defaultVersion, destinationDir, onExtractFileListener);
    }

    private static void doExtractFile(Type type, Context context, String identifier, String defaultVersion, File destination, TarCompressorUtils.OnExtractFileListener onExtractFileListener) {
