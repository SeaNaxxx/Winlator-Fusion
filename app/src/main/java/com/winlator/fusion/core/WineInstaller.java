...
public abstract class WineInstaller {
    public static void generateWineprefix(WineInfo wineInfo, XEnvironment environment) {
        ...
        boolean isProton = wineInfo.type != null && wineInfo.type.equals("proton");

        if (isProton) {
            BionicProgramLauncherComponent bionicProgramLauncherComponent = environment.getComponent(BionicProgramLauncherComponent.class);
            bionicProgramLauncherComponent.setWineInfo(wineInfo);
            bionicProgramLauncherComponent.setBox64Preset(Box64Preset.STABILITY);
            bionicProgramLauncherComponent.setGuestExecutable("wine explorer /desktop=shell,"+ Container.DEFAULT_SCREEN_SIZE+" C:\\windows\\system32\\winecfg.exe");
    ...
    public static ArrayList<WineInfo> getInstalledWineInfos(Context context) {
        return getInstalledWineInfos(context, null);
    }

    public static ArrayList<WineInfo> getInstalledWineInfos(Context context, String containerVariant) {
        ArrayList<WineInfo> wineInfos = new ArrayList<>();
        boolean bionicOnly = Container.BIONIC.equals(containerVariant);
