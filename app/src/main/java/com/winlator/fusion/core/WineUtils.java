...
public abstract class WineUtils {
    ...
    public static void applySystemTweaks(Context context, WineInfo wineInfo) {
        applySystemTweaks(context, wineInfo, null);
    }

    public static void applySystemTweaks(Context context, WineInfo wineInfo, File rootDir) {
        if (rootDir == null) rootDir = RootFS.find(context).getRootDir();
        ...
        if (!wineDir.isDirectory() && wineInfo != null && wineInfo.path != null) {
            ...
            if (protonWineDir.isDirectory()) wineDir = protonWineDir;
        }
        File wineSystem32Dir = new File(wineDir, wineInfo != null && wineInfo.isArm64EC() ? "aarch64-windows" : "x86_64-windows");
        File wineSysWoW64Dir = new File(wineDir, "i386-windows");
        ...
        try (WineRegistryEditor registryEditor = new WineRegistryEditor(userRegFile)) {
            ...
            registryEditor.setStringValue("Software\\Wine\\AddonsURL", null, "https://raw.githubusercontent.com/SeaNaxxx/Winlator-Fusion/dev/wine_addons/");
    ...
    public static void updateWineprefix(Context context, Container container, final Callback<Integer> terminationCallback) {
        RootFS rootFS = RootFS.find(context);

        boolean isBionic = container != null && container.isBionic();
        WineInfo wineInfo = container != null ? WineInfo.fromIdentifier(context, container.getWineVersion()) : null;

        if (isBionic) {
            rootFS = RootFS.fromDir(ImageFs.find(context).getRootDir());
        }
        ...
        if (isBionic) {
            BionicProgramLauncherComponent bionicProgramLauncherComponent = new BionicProgramLauncherComponent(new ContentsManager(context));
            bionicProgramLauncherComponent.setEnvVars(envVars);
            bionicProgramLauncherComponent.setGuestExecutable("wine wineboot -u");
            bionicProgramLauncherComponent.setWineInfo(wineInfo);
            bionicProgramLauncherComponent.setContainer(container);
            bionicProgramLauncherComponent.setTerminationCallback((status) -> {
                FileUtils.writeString(new File(rootDir, RootFS.WINEPREFIX+"/.update-timestamp"), "disable\n");
                if (terminationCallback != null) terminationCallback.call(status);
            });
            environment.addComponent(bionicProgramLauncherComponent);
        } else {
    ...
    private static void setupSystemFonts(WineRegistryEditor registryEditor) {
        ...
        final String[][] wineFonts = {
            ...
            {"Tahoma (TrueType)", wineFontsPrefix + "tahoma.ttf"},
            {"Tahoma Bold (TrueType)", wineFontsPrefix + "tahomabd.ttf"},
            {"Wingdings (TrueType)", wineFontsPrefix + "wingding.ttf"}
        };
