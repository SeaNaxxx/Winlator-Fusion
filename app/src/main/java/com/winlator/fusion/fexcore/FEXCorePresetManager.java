...
public abstract class FEXCorePresetManager {
    ...
    private static String[] splitPresetEntry(String entry) {
        if (entry.contains(FIELD_SEP)) {
            String[] parts = entry.split(FIELD_SEP, 3);
    ...
    private static Iterable<String[]> customPresetsIterator(Context context) {
        ...
        return () -> new Iterator<String[]>() {
            ...
            @Override
            public String[] next() {
                String entry = customPresets[index[0]++];
                return splitPresetEntry(entry);
            }
        };
    }
    ...
    public static void editPreset(Context context, String id, String name, EnvVars envVars) {
        ...
        if (id != null) {
            ...
            for (int i = 0; i < customPresets.length; i++) {
                String[] preset = splitPresetEntry(customPresets[i]);
                if (preset.length > 0 && preset[0].equals(id)) {
                    customPresets[i] = id+FIELD_SEP+name+FIELD_SEP+envVars.toString();
                    break;
                }
            }
            ...
            String preset = FEXCorePreset.CUSTOM+"-"+getNextPresetId(context)+FIELD_SEP+name+FIELD_SEP+envVars.toString();
    ...
    public static void removePreset(Context context, String id) {
        ...
        for (int i = 0; i < customPresets.length; i++) {
            String[] preset = splitPresetEntry(customPresets[i]);
            if (preset.length > 0 && !preset[0].equals(id)) newCustomPresetsStr += (!newCustomPresetsStr.isEmpty() ? "," : "")+customPresets[i];
        }
        ...
        for (int i = 0; i < customPresets.length; i++) {
            ...
            if (preset.length > 0 && preset[0].equals(id)) {
                ...
                if (uriPath != null) {
                    ...
                    presetFile = new File(com.winlator.fusion.SettingsFragment.DEFAULT_WINLATOR_PATH, "Presets/fexcore_" + preset[1] + ".wbp");
                }
    ...
    public static void importPreset(Context context, InputStream stream) {
        ...
        try {
            ...
            customPresetStr = customPresetStr + (!customPresetStr.equals("") ? "," : "") + FEXCorePreset.CUSTOM+"-"+getNextPresetId(context) + FIELD_SEP + preset[1] + FIELD_SEP + preset[2];
        } catch (IOException e) {}
    ...
    public static String migrateFromLegacyValues(String tsoPreset, String x87Mode, String multiblock) {
        boolean tsoEnabled, vectorTSO, memcpyTSO, halfBarrier;
        boolean x87Reduced;
        boolean multiblockEnabled;

        switch (tsoPreset) {
            case "Fastest":
                tsoEnabled = false; vectorTSO = false; memcpyTSO = false; halfBarrier = false;
                break;
            case "Fast":
                tsoEnabled = true; vectorTSO = false; memcpyTSO = false; halfBarrier = false;
                break;
            case "Slow":
                tsoEnabled = true; vectorTSO = false; memcpyTSO = false; halfBarrier = true;
                break;
            case "Slowest":
                tsoEnabled = true; vectorTSO = true; memcpyTSO = true; halfBarrier = false;
                break;
            default:
                tsoEnabled = true; vectorTSO = false; memcpyTSO = false; halfBarrier = true;
                break;
        ...
        x87Reduced = x87Mode.equals("Fast");
        multiblockEnabled = multiblock.equals("Enabled");

        if (!tsoEnabled && !vectorTSO && !memcpyTSO && !halfBarrier && x87Reduced && multiblockEnabled) {
            return FEXCorePreset.PERFORMANCE;
        }
        if (tsoEnabled && !vectorTSO && !memcpyTSO && halfBarrier && x87Reduced && multiblockEnabled) {
            return FEXCorePreset.INTERMEDIATE;
        }
        if (tsoEnabled && vectorTSO && memcpyTSO && !x87Reduced && !multiblockEnabled) {
