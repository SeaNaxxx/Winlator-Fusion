package com.winlator.fusion.win32;
...
public class MSLogFont {
    ...
    public MSLogFont fromByteArray(byte[] byteArray) {
        if (byteArray == null || byteArray.length < 92) return this;
        ByteBuffer data = ByteBuffer.wrap(byteArray).order(ByteOrder.LITTLE_ENDIAN);
        height = data.getInt();
        width = data.getInt();
        escapement = data.getInt();
        orientation = data.getInt();
        weight = data.getInt();
        italic = data.get();
        underline = data.get();
        strikeOut = data.get();
        charSet = data.get();
        outPrecision = data.get();
        clipPrecision = data.get();
        quality = data.get();
        pitchAndFamily = data.get();

        StringBuilder sb = new StringBuilder();
        while (data.remaining() > 0) {
            char chr = data.getChar();
            if (chr == '\0') break;
            sb.append(chr);
        }
